    ...
    for i in tqdm(range(n_batch)):
        ...
        for example in examples:
            for _ in range(n_candidates):
                raw_asms.append(ast.literal_eval(example['codeart']))
        ...
        source_inputs = ['<asm_token>\n' + sig for example in examples \
            for sig in example['candidate_signatures']['top_candidates'][:n_candidates]]
        # print(source_inputs)
...
def main():
    ...
    #         token=model_args.token,
    #     )
    dataset = load_from_disk('../data/probed_continue_data/test')
